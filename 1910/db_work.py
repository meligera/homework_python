...
def edit(id, brand, value):
    sqliteConnection = sqlite3.connect('Databases/{}'.format(p))
    cursor = sqliteConnection.cursor()
    print("Connected to SQLite")
    #sqlite_update_query = """Update middle set id = %d and brand = "%s" where value = "%s\""""
    sqlite_update_query = """Update middle set value = "{:s}", brand = "{:s}" where id = {:d}""".format(value, brand, id)
...
def call(user_a):
    if user_a == 'a':
        print('Список БД: ')
        onlyfiles = [f for f in listdir("/home/sirius/homework/1910/Databases/") if
                             isfile(join("/home/sirius/homework/1910/Databases/", f))]
